...
public class ExcelWriter {
	public static void outputSolution(ClassSolution cs,String url) {
	...
	public static void outputAllCourse(ClassSolution cs,String url) {
		HSSFWorkbook wb=new HSSFWorkbook();
			HSSFSheet sheet =wb.createSheet("课程分配表");
			...
			cell3.getCellStyle().setVerticalAlignment(VerticalAlignment.CENTER);
			
			HSSFCell cell4=row0.createCell(4);
			cell4.setCellValue("任课教师名称");
			cell4.getCellStyle().setAlignment(HorizontalAlignment.CENTER);
			cell4.getCellStyle().setVerticalAlignment(VerticalAlignment.CENTER);
			
			ArrayList<int[]> temp=new ArrayList<int[]>();
			for(int i=0;i<cs.allCourse.size();i++) {
				if(cs.allCourse.get(i)[ExcelReader.CLASSROOM_COL]!=-1) {
					temp.add(cs.allCourse.get(i));
				}
			}
			for(int i=0;i<temp.size();i++) {
				int[] course=temp.get(i);
				...
				courseName=ExcelReader.courseNameHashMap.get(course[ExcelReader.NAME_COL]);
				type=ExcelReader.courseTypeHashMap.get(course[ExcelReader.TYPE_COL]);
				classroom=String.valueOf(course[ExcelReader.CLASSROOM_COL]);
				teacherName=ExcelReader.teacherNameHashMap.get(course[ExcelReader.TEACHER_COL]);
				
				int week;
				int daytime;
				switch(course[ExcelReader.TERM_COL]) {
					case 0:
						week=(course[ExcelReader.WEEKTIME1_COL]-1)/4+1;
						time="前八周 周"+String.valueOf(week);
						daytime=(course[ExcelReader.WEEKTIME1_COL]-1)%4+1;
						time+="第"+String.valueOf(daytime)+"课";
						if(course[ExcelReader.WEEKTIME2_COL]!=-1) {
							week=(course[ExcelReader.WEEKTIME2_COL]-1)/4+1;
							time+=",	前八周 周"+String.valueOf(week);
							daytime=(course[ExcelReader.WEEKTIME2_COL]-1)%4+1;
							time+="第"+String.valueOf(daytime)+"课";
						}
						break;
					case 1:
						week=(course[ExcelReader.WEEKTIME1_COL]-1)/4+1;
						time="后八周 周"+String.valueOf(week);
						daytime=(course[ExcelReader.WEEKTIME1_COL]-1)%4+1;
						time+="后"+String.valueOf(daytime)+"课";
						if(course[ExcelReader.WEEKTIME2_COL]!=-1) {
							week=(course[ExcelReader.WEEKTIME2_COL]-1)/4+1;
							time+=",	后八周 周"+String.valueOf(week);
							daytime=(course[ExcelReader.WEEKTIME2_COL]-1)%4+1;
					...
					case 2:
						week=(course[ExcelReader.WEEKTIME1_COL]-1)/4+1;
						time="全学期 周"+String.valueOf(week);
						daytime=(course[ExcelReader.WEEKTIME1_COL]-1)%4+1;
						time+="第"+String.valueOf(daytime)+"课";
						if(course[ExcelReader.WEEKTIME2_COL]!=-1) {
							week=(course[ExcelReader.WEEKTIME2_COL]-1)/4+1;
							time+=" 周"+String.valueOf(week);
							daytime=(course[ExcelReader.WEEKTIME2_COL]-1)%4+1;
							time+="第"+String.valueOf(daytime)+"课";
						}
						break;
				...
				cella.setCellValue(courseName);
				cellb.setCellValue(type);
				cellc.setCellValue(time);
				celld.setCellValue(classroom);
				celle.setCellValue(teacherName);
				}
